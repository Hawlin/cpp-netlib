...
struct async_server_base : server_storage_base, socket_options_base {
  ...
  typedef typename boost::network::http::response_header<
  ...
  async_server_base(ArgPack const& args)
      : server_storage_base(
          args,
          typename mpl::if_<
                       is_same<typename parameter::value_type<ArgPack,
                                                              tag::io_service,
                                                              void>::type,
                               void>,
                       server_storage_base::no_io_service,
                       server_storage_base::has_io_service>::type()),
  ...
  void listen() {
    ...
    if (!listening)
      start_listening();  // we only initialize our acceptor/sockets if we arent already listening
    if (!listening) {
 ...
 private:
  ...
  void handle_stop() {
    ...
    if (stopping)
      service_
          .stop();  // a user may have started listening again before the stop command is reached
  }
  ...
  void handle_accept(asio::error_code const& ec) {
    {
      scoped_mutex_lock stopping_lock(stopping_mutex_);
      if (stopping)
        return;  // we dont want to add another handler instance, and we dont want to know about errors for a socket we dont need anymore
    }
    if (!ec) {
      ...
      acceptor.async_accept(
          ...
          std::bind(&async_server_base<Tag, Handler>::handle_accept,
                    this,
